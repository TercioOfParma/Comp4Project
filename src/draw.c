...
void drawQuestion( questionData **questions , int questionNo , SDL_Renderer *render , TTF_Font *font )
{
	...
	for(i = 1;  i <= NO_ANSWERS;  i++)//this may not work, check when get home!
	{
		questions[ questionNo ]->display[i]= renderText(font, render, questions[questionNo]->answers[i - 1], &wasSuccess);
		questions[ questionNo ]->display[i]->dimensions.x = QUESTION_XPOS;
		questions[ questionNo ]->display[i]->dimensions.y = QUESTION_YPOS + (50 * i) ;
		drawText( questions[ questionNo ]->display[i] , render );
	}
